...
private:
  ...
  Matrix3 _transformationRotatingText, _projectionRotatingText,
...
TriangleExample::TriangleExample(const Arguments &arguments)
  ...
  _font->fillGlyphCache(_cache,
                        ...
                        "23456789:-+,.!°ěäЗдравстуймиΓειασουκόμ ");
  std::tie(_text, std::ignore) =
      Text::Renderer2D::render(*_font, _cache, 0.1295f,
                               "Hello, world!\n"
                               "Ahoj, světe!\n"
                               "Здравствуй, мир!\n"
                               "Γεια σου κόσμε!\n"
                               "Hej Världen!",
                               _vertices, _indices, GL::BufferUsage::StaticDraw,
  ...
  _dynamicText = std::make_unique<Text::Renderer2D>(*_font, _cache, 32.0f,
...
void TriangleExample::drawEvent() {
  ...
  _text_shader
      .setTransformationProjectionMatrix(_projectionRotatingText *
...
void TriangleExample::mouseScrollEvent(MouseScrollEvent &event) {
  ...
  if (event.offset().y() > 0)
    _transformationRotatingText = Matrix3::rotation(1.0_degf) *
...
void TriangleExample::updateText() {
  _dynamicText->render(
      fmt::format("Rotation: {:.2}°\nScale: {:.2}",
                  Float(Deg(Magnum::Math::Complex<float>::fromMatrix(
                                _transformationRotatingText.rotation())
                                .angle())),
